...
public class AdminCoursesPage extends JFrame implements ActionListener {
    ...
    private static final String CURRENT_PAGE="Courses";
    ...
    public AdminCoursesPage(String adminUsername) {
        this.adminUsername=adminUsername==null ? "admin" : adminUsername;
        setTitle("Faculty Management System - Admin Courses");
    ...
    private JPanel buildSidebarPanel() {
        ...
        styleNavButton(lecturersNavButton, false);
        ...
        styleNavButton(coursesNavButton, true);
    ...
    private JPanel buildContentPanel() {
        ...
        JLabel headingLabel=new JLabel("Courses", SwingConstants.CENTER);
        ...
        String[] columnNames={"DB ID", "Course code", "Course name", "Credits", "Lecturer"};
        tableModel=new DefaultTableModel(DBConnector.getAdminCourseRows(), columnNames){
            @Override
        ...
        int scrollPaneWidth=Math.min(adminTable.getPreferredSize().width + scrollBarWidth + 4,(FRAME_WIDTH - SIDEBAR_WIDTH) - 60);
    ...
    private void saveChanges(){
        ...
        if(success){
            ...
            JOptionPane.showMessageDialog(this, "Course changes saved successfully.");
        }else{
    ...
    private void reloadRowsFromDatabase() {
        ...
        Object[][] rows=DBConnector.getAdminCourseRows();
        for(Object[] row : rows){
    ...
    private boolean saveTableRowsToDatabase(){
        java.util.List<DBConnector.AdminCourseRecord> records=new java.util.ArrayList<>();
        for (int row=0; row<tableModel.getRowCount();row++) {
            records.add(new DBConnector.AdminCourseRecord(
                    toInt(tableModel.getValueAt(row, 0)),
                    valueAt(row, 1),
                    valueAt(row, 2),
                    toRequiredInt(row, 3, "Credits"),
                    valueAt(row, 4)
            ));
        }
        return DBConnector.saveAdminCourses(records, deletedIds);
    }
