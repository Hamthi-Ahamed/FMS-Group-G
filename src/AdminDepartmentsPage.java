...
public class AdminDepartmentsPage extends JFrame implements ActionListener {
    ...
    private static final String CURRENT_PAGE ="Departments";
    ...
    public AdminDepartmentsPage(String adminUsername) {
        this.adminUsername = adminUsername == null ? "admin" : adminUsername;
        setTitle("Faculty Management System - Admin Departments");
    ...
    private JPanel buildSidebarPanel() {
        ...
        styleNavButton(departmentsNavButton, true);
    ...
    private JPanel buildContentPanel() {
        ...
        JLabel headingLabel =new JLabel("Departments", SwingConstants.CENTER);
        ...
        String[] columnNames = {"DB ID", "Name", "HOD", "Degree", "No of Staff"};
        tableModel =new DefaultTableModel(DBConnector.getAdminDepartmentRows(), columnNames) {
            @Override
    ...
    private void saveChanges() {
        ...
        if (success) {
            ...
            JOptionPane.showMessageDialog(this, "Department changes saved successfully.");
        } else {
    ...
    private void reloadRowsFromDatabase() {
        ...
        Object[][] rows =DBConnector.getAdminDepartmentRows();
        for (Object[] row : rows) {
    ...
    private boolean saveTableRowsToDatabase() {
        ...
        java.util.List<DBConnector.AdminDepartmentRecord> records = new java.util.ArrayList<>();
        for (int row = 0; row < tableModel.getRowCount(); row++) {
            records.add(new DBConnector.AdminDepartmentRecord(
                    toInt(tableModel.getValueAt(row, 0)),
                    valueAt(row, 1),
                    valueAt(row, 2),
                    valueAt(row, 3),
                    toRequiredInt(row, 4, "No of Staff")
            ));
        }
        return DBConnector.saveAdminDepartments(records, deletedIds);
    }
