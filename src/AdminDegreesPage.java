...
public class AdminDegreesPage extends JFrame implements ActionListener {
    ...
    private static final String CURRENT_PAGE="Degrees";
    ...
    public AdminDegreesPage() {
        this("admin");
    }

    public AdminDegreesPage(String adminUsername) {
        this.adminUsername = adminUsername == null ? "admin" : adminUsername;
        setTitle("Faculty Management System - Admin Degrees");
    ...
    private JPanel buildSidebarPanel() {
        ...
        styleNavButton(degreesNavButton, true);
    ...
    private JPanel buildContentPanel() {
        ...
        JLabel headingLabel = new JLabel("Degrees", SwingConstants.CENTER);
        ...
        String[] columnNames = {"DB ID", "Degree", "Department", "No of Students"};
        tableModel = new DefaultTableModel(DBConnector.getAdminDegreeRows(), columnNames) {
            @Override
    ...
    private void saveChanges() {
        ...
        if(success) {
            ...
            JOptionPane.showMessageDialog(this, "Degree changes saved successfully.");}
    ...
    private void reloadRowsFromDatabase() {
        ...
        Object[][] rows = DBConnector.getAdminDegreeRows();
        for(Object[] row : rows) {
    ...
    private boolean saveTableRowsToDatabase() {
        ...
        java.util.List<DBConnector.AdminDegreeRecord> records = new java.util.ArrayList<>();
        for(int row = 0; row < tableModel.getRowCount(); row++) {
            records.add(new DBConnector.AdminDegreeRecord(
                    toInt(tableModel.getValueAt(row, 0)),
                    valueAt(row, 1),
                    valueAt(row, 2),
                    toRequiredInt(row, 3, "No of Students")
            ));
        }
        return DBConnector.saveAdminDegrees(records, deletedIds);
    }
